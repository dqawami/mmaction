...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        if if_se_block and se_block_reduction > 0:
            self.se_block = SEModule(planes * self.expansion, se_block_reduction)
        else:
            self.se_block = None
...
def make_res_layer(block,
    ...
    se_block_freq = se_block_freq if not isinstance(se_block_freq, int) else (se_block_freq,) * blocks
    assert len(inflate_freq) == blocks
    assert len(nonlocal_freq) == blocks
    assert len(se_block_freq) == blocks
    ...
    layers.append(
        block(
            ...
            if_se_block=(se_block_freq[0] == 1),
            se_block_reduction=se_block_red,
            with_cp=with_cp,
...
class ResNet_I3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.temporal_pool_blocks = temporal_pool_blocks
        ...
        self.style = style
        self.frozen_stages =\
            frozen_stages if not isinstance(frozen_stages, int) else (frozen_stages,) if frozen_stages >= 0 else ()
        self.reset_stages =\
            reset_stages if not isinstance(reset_stages, int) else (reset_stages,) if reset_stages >= 0 else ()
        ...
        self.se_block_freqs = se_block_freq if not isinstance(se_block_freq, int) else (se_block_freq,) * num_stages
        self.se_block_red = se_block_red
        ...
        self.conv1 = nn.Conv3d(num_input_layers, 64, kernel_size=(conv1_kernel_t, 7, 7), stride=(conv1_stride_t, 2, 2),
        ...
        if pool1_kernel_t > 1:
            if pool1_use_avg:
                self.pool1 = nn.Sequential(nn.MaxPool3d(kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1)),
                                           nn.AvgPool3d(kernel_size=(pool1_kernel_t, 1, 1),
                                                        stride=(pool1_stride_t, 1, 1),
                                                        padding=((pool1_kernel_t - 1) // 2, 0, 0))
                                           )
            else:
                self.pool1 = nn.MaxPool3d(kernel_size=(pool1_kernel_t, 3, 3), stride=(pool1_stride_t, 2, 2),
                                          padding=((pool1_kernel_t - 1) // 2, 1, 1))
        else:
            self.pool1 = None
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            res_layer = make_res_layer(
                ...
                se_block_freq=self.se_block_freqs[i],
                se_block_red=self.se_block_red,
                with_cp=with_cp,
                dropout_ratio=dropout_ratio,
                    ...
                    if isinstance(module, NonLocalModule):
                        ...
                        for attr in ['weight', 'bias', 'running_mean', 'running_var']:
                            logging.info("{}.{} loaded from weights file into {}"
                                         .format(name, attr, getattr(rgetattr(resnet2d, name), attr).shape))
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if self.temporal_pool_blocks is not None and i in self.temporal_pool_blocks:
                x = self.pool2(x)
    ...
    def train(self, mode=True):
        ...
        if mode and len(self.frozen_stages) > 0:
